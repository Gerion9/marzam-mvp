...
// Marzam admin only on mutating operations (Marzam Execution Doc §3 — "create/
// delete users"). Reads open to anyAdmin so BlackPrint can audit invitation
// rollout state without ability to mint new users.
...
router.get('/', authenticate, authorize({ anyAdmin: true }), controller.list);
...
router.get('/pending-users', authenticate, authorize({ anyAdmin: true }), controller.listPendingUsers);
