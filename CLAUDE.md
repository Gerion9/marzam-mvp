...
- Para cambios no triviales en `bq-sync` (los 5 jobs corren en orden fijo), `middleware/scopeFilter.js` + `rbac.js`, `visits.stateMachine.js`, o cualquier migración con FK chains: antes de codear, pedir un ASCII data-flow (inputs, outputs, side-effects, decision tree). Para invocación explícita, usar `/ascii-flow [área]`.
